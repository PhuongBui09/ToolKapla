...
function getCandidateModels(baseUrl, preferredModel = null) {
    ...
    coolingModels.sort((left, right) => left.cooldownUntil - right.cooldownUntil);

    return {
        primaryModel,
        models: [...readyModels, ...coolingModels.map((entry) => entry.modelName)],
    };
}
...
function classifyGeminiFailure(statusCode, errorText) {
    ...
    const normalizedMessage = String(apiError?.message || errorText || '');
    ...
    const isMissingModel =
        statusCode === 404 ||
        apiStatus === 'NOT_FOUND' ||
        normalizedMessageLower.includes('not found');
...
async function pipeStreamToResponse(upstreamRes, res) {
    const reader =
        upstreamRes.body && upstreamRes.body.getReader ? upstreamRes.body.getReader() : null;
...
function createGeminiRequestError(failure) {
    const error = new Error(failure?.text || 'Gemini request failed');
    error.status = failure?.status || 500;
    error.contentType = failure?.contentType || 'text/plain; charset=utf-8';
    error.modelName = failure?.modelName || '';
    error.primaryModel = failure?.primaryModel || '';
    error.attemptedModels = Array.isArray(failure?.attemptedModels) ? failure.attemptedModels : [];
    error.rawResponse = failure?.text || '';
    return error;
}
...
async function requestGeminiText(prompt, preferredModel = null) {
    ...
    if (!result.ok) {
        throw createGeminiRequestError(result.failure);
    }
    ...
    return {
        text: extractTextFromGeminiPayload(payload),
        meta: {
            modelUsed: modelName,
            primaryModel,
            fallbackUsed: modelName !== primaryModel,
            attemptedModels,
        },
    };
}
...
async function handleGeminiProxy(req, res) {
    ...
    const upstreamUrl = process.env.GEMINI_API_URL || DEFAULT_UPSTREAM_URL;
    const apiKey = process.env.GEMINI_API_KEY;
    ...
    try {
        ...
        if (!result.ok) {
            ...
            res.setHeader(
                ...
                String(failure.modelName !== failure.primaryModel),
            );
        ...
        res.setHeader('X-Gemini-Model-Fallback', String(modelName !== primaryModel));
