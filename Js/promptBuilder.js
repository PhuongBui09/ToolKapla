...
Dựa trên phần mô tả buổi học dưới đây, hãy viết nhận xét mô tả CỤ THỂ học sinh đã học và đã làm những gì trong buổi học.
...
function buildInstructions(config) {
  ...
  if (config.includeAllObjectives) {
    ...
    instructions.push(
      `- Được phép: Một số nhận xét có thể tập trung vào một vài mục tiêu`
    );
  }
  ...
  if (config.commentVariety === "low") {
    instructions.push(
      `- Các nhận xét có thể tương tự nhau (chỉ thay đổi cách dùng từ)`
    );
  } else {
    instructions.push(
